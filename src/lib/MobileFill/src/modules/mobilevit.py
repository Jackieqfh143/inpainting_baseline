...
from src.lib.MobileFill.src.modules.cnn_utils import *
from src.lib.MobileFill.src.modules.attention import ECAAttention
import torch
...
class EESPBlock(nn.Module):
    ...
    def __init__(self, nIn, nOut, stride=1,d_rates=[1,2,4] , down_method='esp'): #down_method --> ['avg' or 'esp']
        ...
        for i in range(k-1):
            d_rate = d_rates[i]
            self.spp_dw.append(CDilated(2 * nIn, 2 * nIn, kSize=5, stride=stride, groups=nIn, d=d_rate))
