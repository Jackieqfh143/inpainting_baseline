...
from src.data.Lama_mask_gen import RandomSegmentationMaskGenerator,get_mask_generator
...
class Dataset(data.Dataset):
    def __init__(self, image_path, mask_path, mask_mode, target_size, augment=True, training=True,
                 mask_reverse=True, center_crop=False, data_type='resize', min_mask_ratio=5, max_mask_ratio=70,
                  rect_size=64,seed=2023,mask_type = "default",
                 get_seg_mask=False, load_multi_res = False):
        ...
        self.load_multi_res = load_multi_res
    ...
    def load_item(self, index):
        ...
        if self.load_multi_res:
            img_256, img_512 = self.load_multi_imgs(img)
            mask_256, mask_512 = self.load_multi_imgs(mask, Image.Resampling.NEAREST)

            img_256 = self.to_tensor(img_256)
            img_512 = self.to_tensor(img_512)
            mask_256 = self.to_tensor(mask_256)
            mask_512 = self.to_tensor(mask_512)

            return img_256,mask_256,img_512,mask_512
        else:
    ...
    def load_multi_imgs(self, img, resize_type = Image.Resampling.BILINEAR):
