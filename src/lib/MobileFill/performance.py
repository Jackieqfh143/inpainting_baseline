...
from src.models.mobileFill import MobileFill,MobileFill_v3,MobileFill_v5, MobileFill_v10
from src.models.mat import MAT
from src.models.migan import MIGAN
from src.models.comodgan import CoModGAN
...
def load_model():
    model = MobileFill(input_nc=4,target_size=arg.target_size, down_num=4)
    net_state_dict = model.state_dict()
    state_dict = torch.load(arg.model_path, map_location='cpu')["ema_G"]
...
if __name__ == '__main__':
    ...
    migan_model = MIGAN(model_path="./checkpoints/MIGAN/migan_256_ffhq.pt", targetSize=arg.target_size,device=arg.device)
    # migan_model = MIGAN(model_path="./checkpoints/MIGAN/migan_512_places2.pt", targetSize=arg.target_size,device=arg.device)
    # comodgan_model = CoModGAN(model_path="./checkpoints/CoMoDGAN/places/comodgan_512_places2.pt", targetSize=arg.target_size,device=arg.device)
    mat_model = MAT(model_path="./checkpoints/MAT/celeba-hq/CelebA-HQ_256.pkl", targetSize=arg.target_size, device=arg.device)
    ...
    with torch.no_grad():
        for i, batch in tqdm(enumerate(test_dataloader)):
            ...
            for j in range(arg.sample_num):
                # z = torch.randn(gt.size()[0], 512).to(arg.device)
                # inpaintingModel.encoder.mask_ratio = min(0.5 + random.random(), 0.8)
            ...
            mat_out_imgs = []
            for j in range(arg.sample_num):
                out = mat_model.forward(gt, mask)
                mat_out_imgs.append(out)

            mat_out_imgs = torch.cat(mat_out_imgs, dim=0)
            #
            migan_out_imgs = []
            for j in range(arg.sample_num):
                out = migan_model.forward(gt, mask)
                migan_out_imgs.append(out)

            migan_out_imgs = torch.cat(migan_out_imgs, dim=0)

            # comodgan_out_imgs = []
            # for j in range(arg.sample_num):
            #     out = comodgan_model.forward(gt, mask)
            #     comodgan_out_imgs.append(out)
            #
            # comodgan_out_imgs = torch.cat(comodgan_out_imgs, dim=0)

            time_span += (time.time() - start_time) / (input_x.size(0) * arg.sample_num)
            post_process(out_imgs, gt, mask,i * arg.batch_size,save_path,"MobileFill", arg.sample_num)
            post_process(migan_out_imgs, gt, mask, i * arg.batch_size, save_path, "MIGAN", arg.sample_num)
            # post_process(comodgan_out_imgs, gt, mask, i * arg.batch_size, save_path, "CoModGAN", arg.sample_num)
            post_process(mat_out_imgs, gt, mask, i * arg.batch_size, save_path, "MAT", arg.sample_num)
    ...
    show_im2_html(web_title = f"Result_{arg.dataset_name}",
                  ...
                  max_num = 200,
                  model_list = ["MobileFill", "MIGAN" ,"MAT"])
    ...
    #               im_size=arg.target_size,
    #               max_num=200,
    #               model_list=["CoModGAN","MAT"])

    # print(f"Inference speed: {infer_speed} ms/ img")

    print("Start Validating...")
    print("Performance of MobileFill: \n")
    validate(real_imgs_dir=save_path,
            ...
            get_LPIPS=True,
            get_IDS=True,
            real_suffix=["obileFill_*_im_truth"],
            fake_suffix=["obileFill_*_im_out_0"])

    print("Performance of MIGAN: \n")
    validate(real_imgs_dir=save_path,
             comp_imgs_dir=save_path,
             device=arg.device,
             get_FID=True,
             get_LPIPS=True,
             get_IDS=True,
             real_suffix=["IGAN_*_im_truth"],
             fake_suffix=["IGAN_*_im_out_0"])

    # print("Performance of CoModGAN: \n")
    # validate(real_imgs_dir=save_path,
    #          comp_imgs_dir=save_path,
    #          device=arg.device,
    #          get_FID=True,
    #          get_LPIPS=True,
    #          get_IDS=True,
    #          real_suffix=["oModGAN_*_im_truth"],
    #          fake_suffix=["oModGAN_*_im_out_0"])

    print("Performance of MAT: \n")
    validate(real_imgs_dir=save_path,
             comp_imgs_dir=save_path,
             device=arg.device,
             get_FID=True,
             get_LPIPS=True,
             get_IDS=True,
             real_suffix=["AT_*_im_truth"],
             fake_suffix=["AT_*_im_out_0"])
