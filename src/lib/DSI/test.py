...
from DSI.net.vqvae import vq_encoder_spec, vq_decoder_spec
from DSI.net.structure_generator import structure_condition_spec, structure_pixelcnn_spec
from DSI.net.texture_generator import texture_generator_spec, texture_discriminator_spec
import net.nn as nn
...
import yaml
argsDict = {argName: getattr(args, argName) for argName in vars(args)}
argsDict.update({"vq_encoder_opt":vq_encoder_opt,"vq_decoder_opt":vq_decoder_opt,
                 "structure_condition_opt":structure_condition_opt,"structure_pixelcnn_opt":structure_pixelcnn_opt,
                 "texture_generator_opt":texture_generator_opt,"texture_discriminator_opt":texture_discriminator_opt})

with open('./dsi_configs.yaml', 'w', encoding='utf-8') as f:
    # set allow_unicode=True if contains any Chinese letters
    yaml.dump(argsDict, stream=f, allow_unicode=True)
