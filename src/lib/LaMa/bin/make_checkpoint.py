...
def get_checkpoint_files(s):
    ...
    return 'best.ckpt' if s == 'last' else f'{s}.ckpt'
...
def main(args):
    ...
    shutil.copy2(os.path.join(args.indir, 'configs.yaml'),
                 os.path.join(args.outdir, 'configs.yaml'))
...
if __name__ == '__main__':
    ...
    aparser.add_argument('indir',
                         help='Path to directory with output of training '
                              '(i.e. directory, which has samples, rfr_modules, configs.yaml and train.log')
