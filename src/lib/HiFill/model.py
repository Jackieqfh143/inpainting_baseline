...
def pre_process(raw_img, raw_mask, multiple):
    ...
    small_img = raw_img.copy()
    small_mask = 1. - raw_mask
...
def get_HiFill_output(raw_img,raw_mask,hf_graph,hf_sess,im_ph,mask_ph,inpainted_node,att_node,mask_node):
    with tf.device('/device:GPU:0'):
        with hf_graph.as_default():
            inpainted = inpaint(raw_img, raw_mask, hf_sess, inpainted_node, att_node, mask_node, im_ph,
                                mask_ph, multiple=6)
