...
from src.lib.SPL.rn import RN_B, RN_L, SPADE
...
class G_Net(nn.Module):
    ...
    def forward(self, x, mask, masked_512, mask_512):
        ...
        x = self.encoder(x_input, mask) #(1,256,64,64)

        # perform feature recon
        x_sm = self.encoder_sm(torch.cat((masked_512, mask_512), 1)) #(1,256,64,64)
        x_sm_skip = self.encoder_sm_out(x_sm)#(1,256,64,64)
        local_features = []
        for i in range(self.D):
            x_sm_skip = self.rdbs[i](x_sm_skip)
            local_features.append(x_sm_skip)

        x_sm = self.gff(torch.cat(local_features, 1)) + x_sm #(1,256,64,64)
