...
class testset(torch.utils.data.Dataset):
    def __init__(self, img_list_file,mask_list_file, img_transform, mask_transform,
                 return_name=False):
        ...
        self.img_data = glob(os.path.join(img_list_file,'*.jpg')) + glob(os.path.join(img_list_file,'*.png'))
        self.img_data.sort()
        self.mask_data = glob(os.path.join(mask_list_file,'*.jpg')) + glob(os.path.join(mask_list_file,'*.png'))
        self.mask_data.sort()
    ...
    def __getitem__(self, index):
        ...
        mask = self.mask_transform(mask.convert('RGB'))
        if self.return_name:
            return gt_img * (1-mask), (1 - mask), gt_img, gt_name
        else:
