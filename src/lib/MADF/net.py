...
from src.lib.MADF.normalization import PN
...
class MADFNet(nn.Module):
    def __init__(self, n_refinement_D, layer_size=7, input_channels=3, upsampling_mode=upsampling):
        super().__init__()
        self.upsampling_mode = upsampling_mode
        self.layer_size = layer_size
        self.n_refinement_D = n_refinement_D
        ###Encoder
...
if __name__ == '__main__':
    model = MADFNet(layer_size=7, n_refinement_D=2)
    h = 512
    im = torch.randn(1,3,h,h)
    mask = torch.randn(1,3,h,h)
    from complexity import flop_counter

    flop_counter(model,[im,mask])
    # size = (1, 3, 5, 5)
