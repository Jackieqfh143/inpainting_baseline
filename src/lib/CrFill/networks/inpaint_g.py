...
from src.lib.CrFill.networks.base_network import BaseNetwork
from src.lib.CrFill.networks.utils import gen_conv, gen_deconv, dis_conv
from src.lib.CrFill.networks.splitcam import ReduceContextAttentionP1, ReduceContextAttentionP2
import yaml
...
if __name__ == "__main__":
    config_path = '/home/codeoops/CV/InPainting/Inpainting_baseline/compare/configs/crfill.yaml'
    with open(config_path,'r') as f:
        opt = yaml.load(f.read(),Loader=yaml.FullLoader)
    h = 512
    x = torch.randn(1,3,h,h)
    mask = torch.randn(1,1,h,h)
    model = BaseConvGenerator(opt)
    model(x,mask)
