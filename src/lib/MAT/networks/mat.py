...
from src.lib.MAT.torch_utils import misc
from src.lib.MAT.torch_utils import persistence
from src.lib.MAT.networks.basic_module import FullyConnectedLayer, Conv2dLayer, MappingNet, MinibatchStdLayer, DisFromRGB, DisBlock, StyleConv, ToRGB, get_style_code
...
class Decoder(nn.Module):
    ...
    def forward(self, x, ws, gs, E_features, noise_mode='random'):
        ...
        for res in range(5, self.res_log2 + 1):
            ...
            x, out_img = block(x, img, ws, gs, E_features, noise_mode=noise_mode)
...
class SynthesisNet(nn.Module):
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random', return_stg1=False):
        ...
        stg1_merged = images_in * masks_in + out_stg1 * (1 - masks_in)
        # encoder
        ...
        # decoder
        img, all_imgs = self.dec(fea_16, ws, gs, E_features, noise_mode=noise_mode)

        # ensemble
        img = img * (1 - masks_in) + images_in * masks_in

        return img, all_imgs, stg1_merged
        # if not return_stg1:
        #     return img, all_imgs, stg1_merged
        # else:
        #     return img, stg1_merged
...
class Generator(nn.Module):
    ...
    def forward(self, images_in, masks_in, z, c, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False,
        ...
        img, all_imgs, stg1_merged = self.synthesis(images_in, masks_in, ws, noise_mode=noise_mode)
        return img, all_imgs, stg1_merged
        # if not return_stg1:
        #
        #     return img
