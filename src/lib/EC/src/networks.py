...
class InpaintGenerator(BaseNetwork):
    ...
    def forward(self, x):
        ...
        x = self.middle(x)
        x = self.decoder(x)
        # x = (torch.tanh(x) + 1) / 2     #output x is in 0~1
        # x = (torch.tanh(x) + 1.0) * 0.5  # output x is in 0~1
        return torch.tanh(x)
...
class Discriminator(BaseNetwork):
    ...
    def forward(self, x):
        ...
        return outputs, [conv1, conv2, conv3, conv4, conv5]     #output feature for feature matching
...
if __name__ == '__main__':
    ...
    edgeModel = EdgeGenerator()
    print_network_params(model,'generator')
    print_network_params(edgeModel, 'edgeModel')
    flop_counter(model,x)
    flop_counter(edgeModel,x2)
