from src.lib.EFill.modules.networks import *
import numpy as np
...
# nc_reduce = 2
# without SPADE
class DistInpaintModel_LFFC_Base(BaseNetwork):
    def __init__(self, input_nc = 5, output_nc = 3, ngf=64, n_downsampling=3, n_blocks=9, merge_blocks=3,
                 norm_layer='bn', padding_mode='reflect', activation='relu',
                 out_act='tanh', max_features=512, nc_reduce=2, ratio_g_in=0.5, ratio_g_out=0.5,
        ...
        if self.is_training:
            ...
            edge_out_model = [nn.ReflectionPad2d(2),
                              GatedConv2dWithAct(ngf // 2, 1, kernel_size=5, stride=1, padding=0,
                                                 norm_layer='', activation='', mask_conv='depthConv', conv='normal')]
            edge_out_model.append(actLayer(kind=out_act))
            self.edge_out_l = nn.Sequential(*edge_out_model)
        ...
        out_model = [nn.ReflectionPad2d(2),
                     GatedConv2dWithAct(ngf, output_nc, kernel_size=5, stride=1, padding=0,
                                        norm_layer='', activation='', mask_conv='depthConv', conv='normal')]
        out_model.append(actLayer(kind=out_act))
        self.out_l = nn.Sequential(*out_model)
    ...
    def forward(self, x):
        ...
        if self.is_training:
            ...
            merged_feat = torch.cat((x_l, x_g), dim=1)
...
# then inject it by SPADE into local detail generation
# nc_reduce = 2
class DistInpaintModel_SPADE_IN_LFFC_Base_wo_lwgc(BaseNetwork):
    def __init__(self, input_nc = 5, output_nc = 3, ngf=64, n_downsampling=3, n_blocks=9, merge_blocks=3,
                 norm_layer='bn', padding_mode='reflect', activation='relu',
                 out_act='tanh', max_features=512, nc_reduce=2, ratio_g_in=0.5, ratio_g_out=0.5,
                 selected_edge_layers=[], selected_gt_layers=[], enable_lfu=False, is_training=True):
        ...
        if self.is_training:
            ...
            for i in range(n_downsampling):
                mult = 2 ** (n_downsampling - i - 1)
                model = MyDeConv2d(min(max_features, ngf * mult),
                                   min(max_features, int(ngf * mult / 2)),
                                   kernel_size=3, stride=2, padding=1,
                                   norm_layer=norm_layer, activation=activation
                                   )
            ...
            edge_out_model = [nn.ReflectionPad2d(2),
                              MyConv2d(ngf // 2, 1, kernel_size=5, stride=1, padding=0,
        ...
        out_model = [nn.ReflectionPad2d(2),
                     MyConv2d(ngf // 2, output_nc, kernel_size=5, stride=1, padding=0,
                              norm_layer='', activation='')]
        out_model.append(actLayer(kind=out_act))
        self.out_l = nn.Sequential(*out_model)
...
# baseline model
class DistInpaintModel_woLFFC(BaseNetwork):
    def __init__(self, input_nc = 5, output_nc = 3, ngf=64, n_downsampling=3, n_blocks=9, merge_blocks=3,
                 norm_layer='bn', padding_mode='reflect', activation='relu',
                 out_act='tanh', max_features=256, nc_reduce=2, ratio_g_in=0.5, ratio_g_out=0.5,
                 selected_edge_layers=[], selected_gt_layers=[], enable_lfu=False, is_training=True):
        assert (n_blocks >= 0)
        super().__init__()
        self.is_training = is_training
        ...
        self.selected_gt_layers = selected_gt_layers
        self.en_l0 = nn.Sequential(nn.ReflectionPad2d(2),
                                   GatedConv2dWithAct(in_channels=input_nc, out_channels=ngf, kernel_size=5, padding=0,
                                                      norm_layer=norm_layer, activation=activation))

        ### downsample
        for i in range(n_downsampling):
            mult = 2 ** i
            model = GatedConv2dWithAct(min(max_features, ngf * mult),
                                       min(max_features, ngf * mult * 2),
                                       kernel_size=3, stride=2, padding=1,
                                       norm_layer=norm_layer, activation=activation)
            setattr(self, f"en_l{i + 1}", model)
        ...
        ### resnet blocks
        for i in range(n_blocks):
            cur_resblock = Multidilated_ResBlock(channels=feats_num_bottleneck, dilated_num=4, kernel_size=3,
                                                 norm_layer=norm_layer, activation=activation)
            setattr(self, f"en_res_l{i}", cur_resblock)
    ...
    def forward(self, x):
        ...
        if self.is_training:
            ...
            x2edge = x.clone()
            for i in range(self.num_down):
                x2edge = getattr(self, f'edge_de_l{i}')(x2edge)
                feats_dict[f'edge_de_l{i}'] = x2edge

            for i in range(self.num_down):
                x = getattr(self, f'de_l{i}')(x)
                feats_dict[f'de_l{i}'] = x

            out_edge = self.edge_out_l(x2edge)
            out_edge = (out_edge + 1) / 2.0
            selected_gt_feats = {k: feats_dict.get(k) for k in self.selected_gt_layers}
            selected_edge_feats = {k: feats_dict.get(k) for k in self.selected_edge_layers}
            feats_dict.clear()
...
if __name__ == '__main__':
    from src.lib.EFill.complexity import cuda_infer_time
