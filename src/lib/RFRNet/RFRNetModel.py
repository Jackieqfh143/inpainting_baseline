...
from src.lib.RFRNet.partialconv2d import PartialConv2d
from src.lib.RFRNet.Attention import AttentionModule
from torchvision import models
from tensorboardX import SummaryWriter
import os


#vgg16用于特征提取计算perceptual loss 和 style loss
class VGG16FeatureExtractor(nn.Module):
    def __init__(self):
        super().__init__()
        vgg16 = models.vgg16(pretrained=True)
        #vgg16的输出为['features', 'classifier']
        #这里只取特征提取的部分
...
class RFRModule(nn.Module):
    def __init__(self, layer_size=6, in_channel = 64):
        ...
        self.att = AttentionModule(512)         #KCA模块
    ...
    def forward(self, input, mask):
        ...
        for i in range(self.layer_size - 1, 0, -1):
            ...
            if i == 3:
                h = self.att(h, mask)       #第二个Decoder之后插入KCA模块
        return h
...
class RFRNetModel(nn.Module):
    ...
    def forward(self, in_image, mask):
        ...
        x3 = (x3*m3).mean(dim = 2) /(amp_vec+1e-7)      #feature merge
        x3 = x3.view(n, c, h, w)
        m3 = m3[:,:,-1,:,:]                             #取最后一次递归的Mask
        x4 = self.Tconv(x3)
        x4 = F.leaky_relu(self.bn3(x4), inplace = True)
        m4 = F.interpolate(m3, scale_factor = 2)        #上采样mask使得mask和fearture map大小保持一致
        ...
        if finetune:
            for name, module in self.named_modules():
                if isinstance(module, nn.BatchNorm2d):
                    module.eval()       #模型finetune的时候要固定batchnorm层，防止干扰
...
if __name__ == '__main__':
    ...
    model_input_1 = torch.zeros(1,3,256,256)
    model_input_2 = torch.ones(1,3,256,256)

    model = RFRNet()
    print("model structure: \n",model)
    with SummaryWriter('model_log') as w:
        w.add_graph(model,[model_input_1,model_input_2],True)
