...
class PartialConv2d(nn.Conv2d):
    ...
    def forward(self, input, mask=None):
        ...
        #torch.mul 为矩阵点乘，对应位置元素相乘(而非常规矩阵乘法)
        #输入的mask中,0表示缺失区域而1表示背景,raw_out相当于保存了unmasked 的背景区域
        #raw_out 的shape 不变，还是(input.shape[0],input.shape[1])
        ...
        if self.bias is not None:
            ...
            #要求mask_ratio的shape 和raw_out的shape保持一致
