...
from src.evaluate.fid import calculate_activation_statistics, calculate_frechet_distance
from src.evaluate.inception import InceptionV3
...
class miniDataset(Dataset):
    def __init__(self,real_img_dir,fake_img_dir,real_suffix=['_real'],fake_suffix=['_out','_out_0']):
...
class miniDataset_v2(Dataset):
    def __init__(self,img_dir):
        super(miniDataset_v2, self).__init__()
        self.load_list(img_dir)

    def __len__(self):
        return len(getattr(self, "imgs_0"))
    ...
    def load_item(self,idx):
        ...
        for k in range(self.sample_num):
            im_path = getattr(self, f"imgs_{k}")[idx]
            im = Image.open(im_path).convert('RGB')
            out_items.append(self.to_tensor(im))
        return out_items
    ...
    def load_list(self,img_path):
        im_paths = sorted(glob.glob(img_path+f'/*.jpg') + glob.glob(img_path+f'/*.png'))
        filterd_im_paths = {}

        for path in im_paths:
            im_full_name = get_file_info(path)['file_full_name']
            tmp = re.findall(r"\d+", im_full_name)
            if len(tmp) > 1:
                if filterd_im_paths.get(tmp[-1]) != None:
                    filterd_im_paths[tmp[-1]] += [path]
                else:
                    filterd_im_paths[tmp[-1]] = [path]

        self.sample_num = len(list(filterd_im_paths.keys()))
        for k,v in filterd_im_paths.items():
            setattr(self, f"imgs_{k}", v)
...
def validate(real_imgs_dir,comp_imgs_dir,get_LPIPS=True,get_FID=True,get_MLPIPS = False,
             get_IDS=False,batchSize=10,device='cuda:0',real_suffix=['_truth'],fake_suffix=['_out',"_out_0"]):
    ...
    # calculate mlpips
    if get_MLPIPS:
        mlpips = diversity(comp_imgs_dir, batchSize=batchSize,device=device)
        message_full += ' MLPIPS: {:.4f}'.format(mlpips)
        scores['MLPIPS'] = round(mlpips, 4)
    else:
        scores['MLPIPS'] = 0.0
    ...
    if get_FID:
        ...
        with torch.no_grad():
            ...
            if get_IDS:
                ...
                scores['U-IDS'] = round(u_ids, 4)
                message_full += ' P-IDS: {:.3f}'.format(p_ids)
                scores['P-IDS'] = round(p_ids, 4)
...
def diversity(imgs_dir, batchSize = 10, device = "cuda"):
    dataset = miniDataset_v2(imgs_dir)
    data_loader = DataLoaderX(dataset, batch_size=batchSize,
                              shuffle=False, num_workers=8, drop_last=False)

    print(f'\nCalculating Mean LPIPS...')
    lpips_model = lpips.LPIPS(net='alex')
    lpips_model = lpips_model.to(device)
    lpips_ = 0.0
    count = 0
        ...
        for i in range(len(items) - 1):
            ...
            for j in range(i + 1, len(items)):
                ...
                for k in range(a.shape[0]):
                    loss = lpips_model.forward(a[k:k + 1], b[k:k + 1], normalize=True)  # input img should be -1 ~ 1
                    lpips_ += loss.detach().item()
                    count += 1
    ...
    mlpips = round(lpips_, 4)
    print("The image diversity is ", mlpips)
    return mlpips
...
if __name__ == '__main__':
    # real_path = "/home/codeoops/CV/inpainting_baseline/results/Celeba-hq/thick_256/MobileFill"
    # fake_path = "/home/codeoops/CV/inpainting_baseline/results/Celeba-hq/thick_256/MobileFill"
    # print("Start Validating...")
    # validate(real_imgs_dir=real_path,
    #          comp_imgs_dir=fake_path,
    #          device="cuda",
    #          get_FID=True,
    #          get_LPIPS=True,
    #          get_IDS=True,
    #          real_suffix=["*_im_truth"],
    #          fake_suffix=["*_im_out_0"]
    #          )

    img_path = "/home/codeoops/CV/inpainting_baseline/results/Celeba-hq/mobilefill_celeba-hq/thick_256/FcF"
    mean_lpips = diversity(img_path, batchSize=10, device="cuda")

    # img_path = "/home/codeoops/CV/inpainting_baseline/results/Celeba-hq/efill/medium_512/MMT"
    #
    # tmp_scores_dict, *_ = validate(real_imgs_dir=img_path,
    #                                comp_imgs_dir=img_path,
    #                                device="cuda",
    #                                get_FID=True,
    #                                get_LPIPS=True,
    #                                get_MLPIPS=False,
    #                                get_IDS=True,
    #                                real_suffix=["*_im_truth"],
    #                                fake_suffix=[f"*_im_out_0"])
    #
    # print(tmp_scores_dict)
