...
def mode_seeking_loss(img1, img2, ws1, ws2):
    eps = 1 * 1e-5
    loss = torch.mean(torch.abs(img1 - img2)) / torch.mean(torch.abs(ws1 - ws2))
    loss_lz = 1 / (loss + eps)
    return loss_lz
...
def l2_feat_mat(A_feats,B_feats):
    assert A_feats.shape == B_feats.shape
    loss = []
    for i in range(A_feats.size(0)):
        loss.append(F.mse_loss(A_feats[i],B_feats[i]))
...
def normalize_gradient(net_D, x, **kwargs):
    #originate from https://arxiv.org/abs/2109.02235
    """
                     f
    f_hat = --------------------
            || grad_f || + | f |
